...
class SetCriterion(nn.Module):
    ...
    def loss_layers(self, outputs, targets, indices, num_boxes, log=True):
        """Compute the losses related to the layer depth.
           targets dicts must contain the key "layers" containing a tensor of dim [nb_target_boxes].
        """
        assert 'pred_layers' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_layers = outputs['pred_layers'][idx]
        target_layers = torch.cat([t['layers'][i] for t, (_, i) in zip(targets, indices)], dim=0)

        loss_layer = F.l1_loss(src_layers.squeeze(-1), target_layers, reduction='none')
        loss = loss_layer.sum() / num_boxes
