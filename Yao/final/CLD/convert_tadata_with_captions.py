...
def convert_tadata_with_captions(
    ...
    for pf_idx, pf in enumerate(parquet_files):
        ...
        for i in tqdm(range(len(ds)), desc=f"  轉換樣本"):
            item = ds[i]
            ...
            record = {
                ...
                'image': layer_paths,
                # 保留其他欄位（如果需要）
                'canvas_width': item.get('canvas_width', preview_img.width if isinstance(preview_img, Image.Image) else 0),
                'canvas_height': item.get('canvas_height', preview_img.height if isinstance(preview_img, Image.Image) else 0),
                'type': item.get('type', []),
            }
