...
This is a copy of CLD's dataset contract, extended to support "Method A":
if `{data_dir}/data/*.parquet` exists, we load local parquet shards directly.
Otherwise, we fall back to HuggingFace datasets: `artplus/PrismLayersPro`.
"""
...
class LayoutTrainDataset(Dataset):
    def __init__(self, data_dir, split="train"):
        ...
        if len(local_parquet_files) > 0:
            ...
            full_dataset = load_dataset("artplus/PrismLayersPro", cache_dir=data_dir)
        ...
        subsets = []
        for _, indices in category_to_indices.items():
