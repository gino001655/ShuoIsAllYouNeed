...
def set_sdpa_kernel(mode: str):
    """
    Configure PyTorch SDPA/Flash kernels when available.
    mode: "auto" | "flash" | "mem_efficient" | "math"
    """
    ...
    try:
        ...
        elif mode in ("mem_efficient", "mem-efficient", "efficient"):
        ...
        # Older torch or non-cuda env
        pass


def get_input_box(layer_boxes: List[List[int]]) -> List[Tuple[int, int, int, int]]:
    """
    Quantize bbox to 16-pixel grid (align with token grid used in CLD).
    """
    list_layer_box = []
...
def build_layer_mask(n_layers: int, H_lat: int, W_lat: int, list_layer_box: List[Tuple[int, int, int, int]]):
    """
    Build bbox mask in latent space (assume latent scale factor ~8).
    Returns [L,1,H_lat,W_lat]
    """
...
@torch.no_grad()
def encode_target_latents(vae, pixel_bchw: torch.Tensor, dtype: torch.dtype):
    """
    pixel_bchw: [B,L,3,H,W], in [-1,1]
    returns x0: [B,L,C_lat,H_lat,W_lat]
    """
    device = pixel_bchw.device
    bs, n_layers, _, _, _ = pixel_bchw.shape
    dummy_lat = vae.encode(pixel_bchw[:, 0].to(vae.dtype)).latent_dist.sample()
    _, C_lat, H_lat, W_lat = dummy_lat.shape
    x0 = torch.zeros((bs, n_layers, C_lat, H_lat, W_lat), device=device, dtype=dtype)
    for i in range(n_layers):
        lat = vae.encode(pixel_bchw[:, i].to(vae.dtype)).latent_dist.sample()
        lat = (lat - vae.config.shift_factor) * vae.config.scaling_factor
        x0[:, i] = lat.to(dtype=dtype)
    return x0


def get_timesteps_sd3(scheduler, H_lat: int, W_lat: int, num_inference_steps: int, device):
    """
    Use SD3 pipeline retrieve_timesteps with dynamic shifting.
    """
    # image_seq_len for SD3 uses patch_size tokens; approximate with latent spatial.
